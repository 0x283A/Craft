...
extern "C" void map_alloc(Map *map, int dx, int dy, int dz, int mask)
{
	map->dx = dx;
	map->dy = dy;
	map->dz = dz;
...
extern "C" void map_free(Map *map)
{
	free(map->data);
}

extern "C" void map_copy(Map *dst, Map *src)
{
	dst->dx = src->dx;
	dst->dy = src->dy;
	dst->dz = src->dz;
...
extern "C" int map_get(Map *map, int x, int y, int z)
{
	unsigned int index = hash(x, y, z) & map->mask;
	x -= map->dx;
	y -= map->dy;
...
extern "C" void map_grow(Map *map)
{
	Map new_map;
	new_map.dx = map->dx;
	new_map.dy = map->dy;
